...
try:
    if config.max31855 + config.max6675 + config.max31855spi + config.hotpi > 1:
        log.error("choose (only) one converter IC")
        exit()
    if config.hotpi:
        from hotpi import HotPi, HotPiError
        log.info("import HotPi")
    if config.max31855:
    ...
    if config.max31855spi:
        ...
        if config.gpio_heat_top in spi_reserved_gpio:
            raise Exception("gpio_heat_top pin %s collides with SPI pins %s" % (config.gpio_heat_top, spi_reserved_gpio))
        if config.gpio_heat_bottom in spi_reserved_gpio:
            raise Exception("gpio_heat_bottom pin %s collides with SPI pins %s" % (config.gpio_heat_bottom, spi_reserved_gpio))
    if config.max6675:
...
try:
    ...
    GPIO.setup(config.gpio_heat_top, GPIO.OUT)
    GPIO.setup(config.gpio_heat_bottom, GPIO.OUT)
    GPIO.setup(config.gpio_clrwdt, GPIO.OUT)
    ...
    def run(self):
        ...
        while True:
            ...
            if self.state == Oven.STATE_RUNNING:
                GPIO.output(config.gpio_clrwdt, GPIO.HIGH)
                if self.simulate:
                    self.runtime += 0.5
                else:
                ...
                GPIO.output(config.gpio_clrwdt, GPIO.LOW)
        ...
        if value > 0:
            ...
            if gpio_available:
               if config.heater_invert:
                 GPIO.output(config.gpio_heat_top, GPIO.LOW)
                 GPIO.output(config.gpio_heat_bottom, GPIO.LOW)
                 time.sleep(self.time_step * value)
                 GPIO.output(config.gpio_heat_top, GPIO.HIGH)
                 GPIO.output(config.gpio_heat_bottom, GPIO.HIGH)
               else:
                 GPIO.output(config.gpio_heat_top, GPIO.HIGH)
                 GPIO.output(config.gpio_heat_bottom, GPIO.HIGH)
                 time.sleep(self.time_step * value)
...
class TempSensorReal(TempSensor):
    def __init__(self, time_step):
        ...
        if config.hotpi:
            log.info("init HotPi")
            self.thermocouple = HotPi(config.temp_scale)

    def run(self):
        if config.hotpi:
            self.thermocouple.initialise_for_this_thread()
